        ...
        for row in rows:
            ...
            if len(next_url) > 0:
                for url in next_url:
                    ...
                    sql_output = "INSERT INTO output(sku,asin,cost,sellername,shippingcost,description) VALUES (%s,%s,%s,%s,%s,%s)"
                    cur.execute(sql_output, (sku,asin,cost,sellerName,shippingCost,description))
                    con.commit()

                    # sql_url = "INSERT INTO url(url) VALUES (%s)"
                    # cur.execute(sql_url, (str(url))
                    # con.commit()

                    time.sleep(5)

                sql_update = "UPDATE csv SET completed=1 WHERE sku='" + str(sku) + "'"
                cur.execute(sql_update)

                # sql_trun = "TRUNCATE TABLE url"
                # cur.execute(sql_trun)
